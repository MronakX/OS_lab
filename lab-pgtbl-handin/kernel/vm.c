...
kuvmcopy(pagetable_t pagetable, pagetable_t k_pagetable, uint64 sz, uint64 start)
{
  ...
  for(i = start; i < start + sz; i += PGSIZE){
    if((pte = walk(pagetable, i, 0)) == 0)//check if exist in user pgtbl
      panic("uvmcopy: pte should exist");
    if((k_pte = walk(k_pagetable, i, 1)) == 0)//create in k-pgtbl
      panic("uvmcopy: k_pte can't alloc");
    *k_pte = (*pte) & (~PTE_U);//PTE_U bit is 0, to access in kernel mode.
  }
  return 0;
}
...
{
  printf("page table %p\n", pagetable);//print the parameter
  vmprint_with_depth(pagetable, 0);//depth initialize to 0
}
...
void vmprint_with_depth(pagetable_t pagetable, int depth)
{
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V)){ //check if valid
      uint64 child = PTE2PA(pte);
      if(depth == 0)//only three cases, OK to use ifelse
        printf("..%d: pte %p pa %p\n", i, pte, child);
      if(depth == 1)
        printf(".. ..%d: pte %p pa %p\n", i, pte, child);
      if(depth == 2)
        printf(".. .. ..%d: pte %p pa %p\n", i, pte, child);
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0)//it is a leaf node
